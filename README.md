...
v 4.1 | Stand: Juli 2021 | Lizenz: CC BY-NC-SA 4.0

# Kurzfassung
Für Lesefaule, die wissen, was sie tun:
- Windows 10 mit OfflineKonto einrichten (Alle Angebote und Optionen bei der Installation abwählen)
- Updates installieren
- Im BIOS nur Festplatte als Boot-Medium zulassen und Admin-Passwort vergeben
- BIOS updaten
- Nicht benötigte Software deinstallieren
- Unerwünschte Funktionalitäten mit [ShutUp10](https://www.oo-software.com/de/shutup10) deaktivieren
- W-Lan und Bluetooth ggf. deaktivieren
- Festplatte verschlüsseln
- Webcam und Mikrofon deaktivieren
- Einfaches Nutzerkonto oder VM einrichten
- Benötigte Software installieren und einrichten ([TorBrowser](https://www.torproject.org/de/download/), [Firefox](https://www.mozilla.org/de/exp/firefox/new/), [Thunderbird](https://www.thunderbird.net/de/), [LibreOffice](https://de.libreoffice.org/), [VLC](https://www.videolan.org/vlc/), [7zip](https://www.7-zip.org/), [VeraCrypt](https://www.veracrypt.fr/), [KeePassXC](https://keepassxc.org/),...)
- BackUps einrichten

Etwas mehr Details gewünscht? Kein Problem, hier die Langfassung:

# Vorwort
Diese Anleitung soll dabei helfen, Windows 10 sicher einzurichten. Mit *sicher* ist dabei einerseits ein so datenschutzfreundliches und schwer anzugreifendes Windows wie möglich gemeint. Andererseits soll sich diese Anleitung auf Maßnahmen beschränken, die durchschnittliche Anwender:innen nicht überfordern und die alltägliche Benutzung des Systems möglichst wenig beeinflussen. Es geht hier also explizit nicht um ein möglichst gehärtetes System, sondern um so viel Sicherheit, wie ohne größere Einschränkungen eben möglich ist.

*Für sicherheitsempfindliche Einsatzszenarien sollte über einen vollständigen Verzicht auf Windows nachgedacht und stattdessen ein Linux verwendet werden.*
...
Ihre Daten sollten Sie übrigens erst auf das Gerät kopieren bzw. mit dem Computer erst arbeiten, wenn Sie diese Anleitung bis mindestens einschließlich Kapitel 10 durchlaufen haben. Wenn möglich, versuchen Sie das nutzen des Systems komplett zu vermeiden, bis dieses vollständig konfiguriert ist bzw. Sie am Ende dieser Seite angekommen sind.
...
# 1 Der erster Start
*(Diese Anleitung geht davon aus, dass Sie vor einem Windows-System im Auslieferungszustand sitzen. Sollte dies nicht der Fall sein, können Sie einen bereits verwendeten Computer wie folgt wieder zurücksetzen: Start, Einstellungen, Update und Sicherheit, Wiederherstellung, Diesen PC zurücksetzen.)*

- Gerät einschalten (Eine Internetverbindung wird noch nicht benötigt).
- Sprache auswählen.
- Region auswählen.
...
- Weiter mit *Ich habe kein Internet*.
- Weiter mit *eingeschränktem Setup* (lokales Konto). 
(Einschub: Die Home-Version versucht seit 2020 das Einrichten eines Online-Kontos zu erzwingen. Hier haben Sie die Option Offlinekonto nur, wenn Sie das Gerät von Anfang an ohne Verbindung zum Internet aufsetzen und erst NACH der Installation mit einem Netzwerk verbinden.)
...
- Ggf. eingeblendete Angebote des Herstellers bzgl. Garantie etc. überspringen. 
...
## 3.2 BIOS-Update
Suchen Sie auf der Website Ihres Geräteherstellers nach einem Update für Ihr BIOS und installieren Sie dieses gemäß der Anleitung Ihres Herstellers.
...
- Programme&Features: Öffnen Sie das Menü Apps&Features (s.o.) im Vollbild. Oben rechts erscheint nun die Option "Programme und Features". Öffnen Sie dieses und deinstallieren Sie auch dort sämtliche nicht mehr benötigte Software.

Generell gilt: So viel deinstallieren wir möglich! Eine Software, die nicht auf Ihrem System ist, kann Ihnen später auch nicht zum Verhängnis werden. Sollten Sie sich bei einzelnen Programmen unsicher sein, klärt eine kurze Onlinerecherche meist schnell, was sich hinter nicht selbsterklärenden Programmnamen verbirgt.
...
## 8.2 VeraCrypt
Sollten Sie Windows 10 Home nutzen, steht Ihnen BitLocker leider nicht zur Verfügung. Aber keine Sorge: Dank [VeraCrypt](https://www.veracrypt.fr/en/Downloads.html) müssen Sie nicht auf den wichtigen Grundschutz Festplattenverschlüsselung verzichten.
...
## 9.2 Mikrofon
Auch mit dem ggf. eingebauten Mikrofon sollten Sie sich kurz befassen. Sofern Sie dieses gelegentlich benötigen, machen Sie folgendes: Drücken Sie auf das Windows-Logo und geben Sie "Mikrofonzugriff systemweit deaktivieren" ein. In dem sich öffnenden Fenster können Sie den Mikrofonzugriff durch Software generell unterbinden. Geben Sie diesen nur frei, solange Sie diesen auch tatsächlich benötigen.

Sollten Sie Ihr Mikrofon hingegen nie benötigen, gehen Sie noch einen Schritt weiter und deaktivieren Sie dieses generell im bereits erwähnten BIOS ihres Computers.

# 10 Adminrechte loswerden
Ein zentrales Sicherheitsproblem von Windows ist, dass es meist direkt aus dem Administratorkonto heraus verwendet wird. Im Falle einer Infektion mit Schadsoftware wird es dieser dadurch deutlich einfacher gemacht, ebenfalls Administratorrechte auf dem System zu erlangen. Dazu bieten sich im Wesentlichen zwei Wege an:

1. Entweder sie erstellen auf Ihrem PC ein weiteres Nutzerkonto ohne Adminrechte und arbeiten von nun an ausschließlich von diesem aus.
2. Oder aber Sie richten sich virtuelle Maschinen ein und arbeiten in diesen weiter.

Ein zusätzliches Nutzerkonto ist einfach in der Handhabung, Virtualisierung bietet dafür mehr Schutz und mehr Möglichkeiten. Egal für welchen Weg Sie sich entscheiden: Nach diesem Kapitel sollten Sie Ihren ursprünglich auf dem Computer angelegten Nutzer mit seinen Adminrechten nicht mehr nutzen, um im Internet zu surfen, zu arbeiten, oder was auch immer Sie auf Ihrem System machen wollen.

Führen Sie also die nachfolgenden Kapitel für die zukünftig angestrebte Arbeitsumgebung durch: Sei es ein weiterer Windowsnutzer oder eine Virtuelle Maschine.
...
Nach dem Anlegen des Kontos sollte der Rechner neu gestartet und von nun an (wann immer möglich) nur noch über dieses Nutzerkonto verwendet werden. Keine Sorge: Sie können weiterhin Dinge tun, die Administratorenrechte benötigen, z.B. Software installieren. Windows 10 wird Sie in solchen Fällen nach Ihrem Administratorenkennwort fragen und Ihnen kurzzeitig Administratorenrechte einräumen.

## 10.2 Virtuelle Maschinen nutzen
Alternativ können Sie auch einfach Ihre eigentliche Nutzung des Systems in eine oder mehrere virtuelle Maschinen auslagern. So können Sie z.B. dienstliches und privates sauber voneinander trennen. 
...
Sie können sich also zum Beispiel mit Hilfe der Software [VirtualBox](https://www.virtualbox.org/) mehrere virtuelle Maschinen (VM) anlegen, wie es u.a. [hier](https://itsfoss.com/install-linux-mint-in-virtualbox/) erklärt wird. In der einen erledigen Sie ihren privaten Papierkrieg, in der anderen ihre berufliche Arbeit. In einer dritten experimentieren Sie mit neuer Software, in einer vierten besuchen Sie unseriöse Webseiten, etc. Sollten Sie sich in einer VM Schadsoftware einfangen, kann diese nicht die Daten in Ihren anderen VMs erreichen.
...
Nun ist an an der Zeit, die zum Verwenden des Laptops benötigte Software zu installieren. Wann immer möglich, sollte man dabei auf [Freie Software](https://fsfe.org/freesoftware/freesoftware.de.html) zurückgreifen.
...
Die Anonymisierung funktioniert umso besser, je weniger man an seinem TorBrowser verändert hat. Wir empfehlen also, den Tor-Browser in der ausgelieferten Standartkonfiguration zu betreiben.
...
Zur Installation von Add-ons klicken Sie bitte in Ihrem Firefox-Browser auf die drei Balken oben rechts und dann auf *Add-ons*. Hier können Sie nun Erweiterungen aussuchen und installieren. Anbei ein paar Empfehlungen der Autoren:
...
## 11.2 E-Mail-Client
Sofern E-Mails auf Ihrem Computer nicht über den Browser abgerufen werden sollen, empfiehlt sich die Installation von [Thunderbird](https://www.thunderbird.net/de/), um Mails automatisch abrufen und auch offline verarbeiten zu können. Mit der aktuellen Version von Thunderbird wird auch das sichere Verschlüsseln von E-Mails zum Kinderspiel, aber dazu vielleicht in einem anderen Tutorial später mehr.

### 11.2.1 Masterpasswort
...
### 11.2.2 User-Agent
Im Header jeder E-Mail übermittelt Thunderbird u.a. neben dem verwendeten Betriebssystem auch die verwendete Thunderbird-Version. 
Das sieht dann ungefähr so aus:

```
User-Agent: Mozilla/5.0 (X11; Linux armv7l; rv:78.0) Gecko/20100101 Thunderbird/78.10.0
```

Diese Mail stammt also von einem LinuxNutzer mit der Thunderbirdversion 78.10.0... welch hilfreiche Information, um direkt die passende Schadsoftware zu suchen und zurückzuschicken.

Dies lässt sich zum Glück einfach abschalten: Einfach in Thunderbird unter *Einstellungen* -> *Allgemein* -> *Konfiguration Bearbeiten* den Eintrag *general.useragent.override* mit einem Leerzeichen belegen. Das lässt Ergebnis lässt sich übrigens einfach testen: Schreiben Sie vorher und nacher eine Mail an sich selbst. Markieren Sie je eine Mail und drücken Sie dann *Strg* + *U*. Dann öffnet sich der Quelltext, in dem Sie relativ weit oben dann die Einträge unter UserAgent vergleichen können.
...
Wir können Ihnen zum Beispiel [LibreOffice](https://de.libreoffice.org/) wärmstens empfehlen.
...
## 11.5 Weitere Software-Empfehlungen
...
Sofern die zu sichernden Daten nicht größer als ungefähr 100GB sind, ist eine einfache mögliche Lösung, sich einfach 2 USB-Sticks zu besorgen. Mit Hilfe des bereits erwähnten [7Zip](https://www.7-zip.org/) erstellen Sie dann einfach immer Freitags in geraden Wochen auf dem einen und in ungeraden Wochen auf dem anderen Stick ein **verschlüsseltes** Archiv mit allen Ihren Daten.
...
- Mit einer aktuellen Version des verwendeten Tools (siehe Kapitel 5.1) prüfen, ob z.B. durch Updates unerwünschte Funktionalitäten aktiviert wurden.
...
- Windows neu von einem USB-Stick installieren
